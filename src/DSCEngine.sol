...
contract DSCEngine is ReentrancyGuard {
    ...
    mapping(address user => mapping(address token => uint256 amount))
        public userCollateralBalanceWithToken;
    ...
    function getTokenAmountFromUsd(
        ...
        AmountLessThanZero(usdAmountinWei)
        returns (uint256)
    {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(
            tokenPriceFeedMapping[token]
        );
     ...
     */
    function liquidate(
        address tokenCollaterizationAddress,
        address userWhoDefaulted,
        uint256 debtToBeCovered
    )
        public
        AmountLessThanZero(debtToBeCovered)
        UnsupportedToken(tokenCollaterizationAddress)
        nonReentrant
    {
        if (!checkingIfHealthFactorIsBelowThreshold(userWhoDefaulted)) {
            revert DSC_HealthFactorAboveThresholdAndNeedNotToBeLiquidated();
        }
        uint256 startingUserHealthFactor = calcualtingHealthFactor(
            userWhoDefaulted
        );
        uint256 tokenAmountFromDebtCovered = getTokenAmountFromUsd(
            tokenCollaterizationAddress,
            debtToBeCovered
        );
        ...
        uint256 endingUserHealthFactor = calcualtingHealthFactor(
            userWhoDefaulted
        );
    ...
    function burnDsc(uint256 amount) public AmountLessThanZero(amount) {
        _i_burnDsc(amount, msg.sender, address(this));
    ...
    function _i_burnDsc(
        ...
        address defaultUser,
        address dscFrom
    ) private AmountLessThanZero(amount) {
        userDSCBalance[defaultUser] -= amount;
    ...
    function _i_redeemCollateral(
        address token,
        address from,
        address to,
        uint256 amount
    ) private AmountLessThanZero(amount) UnsupportedToken(token) {
        userCollateralBalanceWithToken[from][token] -= amount;
    ...
    function depositCollateral(
        ...
        public
        nonReentrant
        AmountLessThanZero(collateralAmount)
        UnsupportedToken(collateralTokenAddress)
    {
        userCollateralBalanceWithToken[msg.sender][
        ...
        bool successfulTransaction = IERC20(collateralTokenAddress)
    ...
    function mintDsc(
        uint256 _DscAmount
    ) public nonReentrant AmountLessThanZero(_DscAmount) {
        userDSCBalance[msg.sender] += _DscAmount;
        bool healthFactorOk = checkingIfHealthFactorIsBelowThreshold(
            msg.sender
        );
        if (!healthFactorOk) {
            userDSCBalance[msg.sender] -= _DscAmount;
    ...
    function checkingIfHealthFactorIsBelowThreshold(
        address user
    ...
    function calcualtingHealthFactor(
        address user
    ) public view returns (uint256) {
        (uint256 dscBalance, uint256 collateralValue) = getAccountInformation(
            user
        );
        if (dscBalance == 0) {
    ...
    function getUserDscBalance(address user) public view returns (uint256) {
        return userDSCBalance[user];
    }
    ...
    function getCollateralBalanceOfUser(
        ...
        return userCollateralBalanceWithToken[user][token];
    }
}
